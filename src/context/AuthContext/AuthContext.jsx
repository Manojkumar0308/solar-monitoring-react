...
export const AuthProvider = ({ children }) => {
  ...
  const handleLoginClick = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://192.168.1.49:3000/api/user/login',
        { email, password },
  ...
  const verifyUseremail = async()=>{
    ...
    try{
      ...
      const response = await axios.post(  'http://192.168.1.49:3000/api/user/get-verify-user', requestBody,{ headers: { 'Content-Type': 'application/json' } })
